...
import io.smallrye.bayesian.graph.Edge;
import io.smallrye.bayesian.graph.Graph;
import io.smallrye.bayesian.graph.GraphNode;
import io.smallrye.bayesian.util.bitmask.OpenBitSet;
...
public class JunctionTreeBuilder {
    ...
    public JunctionTree build(boolean init) {
        return build( null, null, init );
    }

    public JunctionTree build(String namespace, String name) {
        return build( namespace, name, true );
    }

    public JunctionTree build(String namespace, String name, boolean init) {
        moralize();
        List<OpenBitSet>  cliques = triangulate();
        return junctionTree(namespace, name, cliques, init);
    }
    ...
    public JunctionTree junctionTree(List<OpenBitSet> cliques, boolean init) {
        return junctionTree(null, null, cliques, init);
    }

    public JunctionTree junctionTree(String namespace, String name, List<OpenBitSet> cliques, boolean init) {
        ...
        for ( int i = 0, length = cliques.size(); i < length; i++ ) {
            ...
            mapVarNodeToCliques(varNodeToCliques, i, cliques.get(i), cliques.size());
        }
        ...
        return new JunctionTree(namespace, name, graph, jtNodes[0], jtNodes, jtSeps, init);
    }
    ...
    public void mapNodeToCliqueFamily(OpenBitSet[] varNodeToCliques, JunctionTreeClique[] jtNodes) {
        for ( int i = 0; i < varNodeToCliques.length; i++ ) {
            ...
            OpenBitSet parents = new OpenBitSet(graph.size());
            int count = 0;
     ...
     * @param clique
     */
    public void mapVarNodeToCliques(OpenBitSet[] nodeToCliques, int id, OpenBitSet clique, int numCliques) {
        for ( int i = clique.nextSetBit(0); i >= 0; i = clique.nextSetBit( i + 1 ) ) {
             OpenBitSet cliques = nodeToCliques[i];
            if ( cliques == null ) {
                cliques = new OpenBitSet(numCliques);
